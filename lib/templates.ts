...
export function generateCancellationEmailHTML(event: any, reason?: string): string {
  const meetingDate = new Date(event.start).toLocaleDateString('en-GB', {
    day: '2-digit',
    month: 'short',
    year: '2-digit'
  }).replace(/(\d+)\/(\w+)\/(\d+)/, '$1/$2/\'$3')
  const meetingDay = new Date(event.start).toLocaleDateString('en-US', {
    weekday: 'short'
  })
  const meetingTime = `${new Date(event.start).toLocaleTimeString('en-GB', {
    hour: '2-digit',
    minute: '2-digit',
    hour12: false
